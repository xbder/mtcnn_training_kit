    ...
    def detect_rnet(self, im, dets):
        ...
        for i in range(num_boxes):
            ...
            try:
                ...
                # print("mtcnn/core/detect:处理前tmp ", type(tmp), tmp.shape)
                # print("mtcnn/core/detect:处理前im ", type(im), im.shape)
                # print("tmp:", tmp)
                # tmp[dy[i]:edy[i]+1, dx[i]:edx[i]+1, :] = im[y[i]:ey[i]+1, x[i]:ex[i]+1, :]
                # print("mtcnn/core/detect:处理后tmp ", type(tmp), tmp.shape)
                # print("mtcnn/core/detect:处理后im ", type(im), im.shape)
                # print("tmp:", tmp)
                # print("===============================================")
            except Exception as e:
                print("tmp:", tmp.shape, edy[i]+1-dy[i], dx[i]+1-dx[i])
                print("im", im.shape, ey[i]+1-y[i], ex[i]+1-x[i])
